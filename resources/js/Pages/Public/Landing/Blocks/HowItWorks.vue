...
<template>
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <div class="text-center mb-16">
      ...
      <p class="text-xl text-slate-600 max-w-4xl mx-auto mb-8">
      ...
      <button 
        ...
        class="inline-flex items-center px-8 py-4 bg-gradient-to-r from-sky-600 to-blue-600 hover:from-sky-700 hover:to-blue-700 text-white font-bold rounded-xl shadow-lg hover:shadow-xl transform hover:scale-105 transition-all duration-200 text-lg"
        :class="{ 'opacity-50 cursor-not-allowed': isAnimating }"
      >
        <font-awesome-icon :icon="['fas', 'fa-play']" class="mr-3 text-xl" />
        {{ isAnimating ? 'Watch the Magic...' : 'See How It Works' }}
      </button>
    </div>

    <!-- Interactive Call Workflow -->
    <div class="bg-white rounded-3xl p-8 sm:p-12 shadow-2xl border border-sky-100 mb-20">
      <!-- Visual Workflow -->
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-12">
        <!-- Left Side: Flow Diagram -->
        <div class="space-y-6">
          <!-- Step 1: Call Received -->
          <div class="flex items-center gap-4 p-6 rounded-xl transition-all duration-300"
            ...
            <div class="w-14 h-14 rounded-full flex items-center justify-center transition-colors duration-300"
              ...
              <font-awesome-icon :icon="['fas', 'fa-phone-alt']" class="text-lg" />
            </div>
            <div>
              <h4 class="text-lg font-bold text-slate-800">Customer calls your business number</h4>
          ...
          <div class="flex items-center gap-4 p-6 rounded-xl transition-all duration-300"
            ...
            <div class="w-14 h-14 rounded-full flex items-center justify-center transition-colors duration-300"
              ...
              <font-awesome-icon :icon="['fas', 'fa-mobile-alt']" class="text-lg" />
            </div>
            <div>
              <h4 class="text-lg font-bold text-slate-800">Forward to your mobile</h4>
              <div class="flex items-center gap-2">
                <p class="text-sm text-slate-600">Ring for 20 seconds</p>
                <span class="text-xs px-3 py-1 bg-blue-100 text-blue-600 rounded-full font-medium">Optional</span>
              </div>
            </div>
          </div>
          ...
          <div class="flex items-center gap-4 p-6 rounded-xl transition-all duration-300"
            ...
            <div class="w-14 h-14 rounded-full flex items-center justify-center transition-colors duration-300"
              ...
              <font-awesome-icon :icon="['fas', 'fa-voicemail']" class="text-lg" />
            </div>
            <div>
              <h4 class="text-lg font-bold text-slate-800">Custom voicemail plays if unanswered</h4>
          ...
          <div class="flex items-center gap-4 p-6 rounded-xl transition-all duration-300"
            ...
            <div class="w-14 h-14 rounded-full flex items-center justify-center transition-colors duration-300"
              ...
              <font-awesome-icon :icon="['fas', 'fa-file-text']" class="text-lg" />
            </div>
            <div>
              <h4 class="text-lg font-bold text-slate-800">Voicemail transcribed instantly</h4>
        ...
        <div class="space-y-8">
          <!-- Summary to You -->
          <div class="bg-gradient-to-br from-white to-sky-50 rounded-2xl border-2 p-8 transition-all duration-500 shadow-lg"
               :class="showCallSummary ? 'border-sky-200 shadow-2xl scale-105' : 'border-gray-200 opacity-50'">
            <div class="flex items-center gap-4 mb-6">
              <div class="bg-sky-100 rounded-full w-16 h-16 flex items-center justify-center shadow-md">
                <font-awesome-icon :icon="['fas', 'fa-user-tie']" class="text-sky-600 text-xl" />
              </div>
              <div>
                <h4 class="text-xl font-bold text-slate-800">Smart SMS sent to you</h4>
                <p class="text-sm text-slate-600">Your AI agent summarises everything</p>
              </div>
            </div>

            <div class="bg-white rounded-xl p-4 shadow-sm" v-show="showCallSummary">
              <div class="flex justify-end">
                <div class="bg-blue-500 rounded-2xl rounded-br-md px-4 py-3 max-w-sm">
          ...
          <div class="bg-gradient-to-br from-white to-green-50 rounded-2xl border-2 p-8 transition-all duration-500 shadow-lg"
               :class="showAutoReply ? 'border-green-200 shadow-2xl scale-105' : 'border-gray-200 opacity-50'">
            <div class="flex items-center gap-4 mb-6">
              <div class="bg-green-100 rounded-full w-16 h-16 flex items-center justify-center shadow-md">
                <font-awesome-icon :icon="['fas', 'fa-reply']" class="text-green-600 text-xl" />
              </div>
              <div>
                <h4 class="text-xl font-bold text-slate-800">Auto-reply sent to caller</h4>
                <div class="flex items-center gap-2">
                  <p class="text-sm text-slate-600">Professional quote based on your pricing</p>
                  <span class="text-xs px-3 py-1 bg-green-100 text-green-600 rounded-full font-medium">Optional</span>
                </div>
              </div>
            </div>

            <div class="bg-white rounded-xl p-4 shadow-sm" v-show="showAutoReply">
              <div class="flex justify-end">
                <div class="bg-blue-500 rounded-2xl rounded-br-md px-4 py-3 max-w-sm">
                  <p class="text-sm text-white leading-relaxed">Thanks for calling! Fuse box issues typically $320-450 incl. callout. I'll check your message and get back shortly. If urgent, reply here. - Dave</p>
                  <div class="text-xs text-blue-200 mt-2 flex items-center justify-end">
                    <span>10:19 AM</span>
                    <div class="ml-1 text-white">✓✓</div>
                  </div>
