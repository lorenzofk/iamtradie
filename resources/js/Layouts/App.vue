  ...
  <div class="flex min-h-screen overflow-x-hidden">
    ...
    <div class="lg:hidden fixed top-0 left-0 right-0 z-50 bg-white border-b border-gray-200 px-4 py-4">
      <div class="flex items-center justify-between">
        ...
        >
          <font-awesome-icon :icon="['fas', isMobileMenuOpen ? 'fa-times' : 'fa-bars']" class="h-5 w-5" />
        </button>
      </div>
    </div>

    <!-- Mobile menu overlay -->
    <div v-if="isMobileMenuOpen" class="lg:hidden fixed inset-0 z-40 flex" @click="closeMobileMenu">
      <!-- Backdrop -->
      ...
      <div class="relative flex-1 flex flex-col max-w-64 w-full bg-white" @click.stop>
        <div class="flex flex-col h-full pt-20">
          ...
          <div class="flex-shrink-0 flex border-t border-gray-200 p-4 items-center w-full bg-white">
            ...
            <button
              @click="onLogout"
              class="cursor-pointer ml-2 text-gray-400 hover:text-gray-600 transition-colors flex-shrink-0 p-1"
            >
